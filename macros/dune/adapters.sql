...
{% macro trino__get_columns_in_relation(relation) -%}
  {% do log('trino__get_columns_in_relation', info=true) %}
  {%- set sql -%}
    describe {{ relation }}
  {%- endset -%}
...
{% macro trino__list_relations_without_caching(relation) %}
  {% do log('trino__list_relations_without_caching', info=true) %}
...
{% macro trino__reset_csv_table(model, full_refresh, old_relation, agate_table) %}
    {% do log('trino__reset_csv_table', info=true) %}
...
{% macro trino__create_csv_table(model, agate_table) %}
  {% do log('trino__create_csv_table', info=true) %}
...
{% macro properties(properties, relation) %}
  {% do log(properties, info=true) %}
  {% do log(relation, info=true) %}
  {%- if properties is not none -%}
      WITH (
          location = 's3a://trino-dev-datasets-118330671040/hive/{{relation.schema}}/{{relation.identifier}}'
...
{% macro trino__create_table_as(temporary, relation, sql) -%}
  {% do log('trino__create_table_as', info=true) %}
  {%- set _properties = config.get('properties') -%}
...
{% macro trino__create_view_as(relation, sql) -%}
  {% do log('trino__create_view_as', info=true) %}
...
{% macro trino__drop_relation(relation) -%}
  {% do log('trino__drop_relation', info=true) %}
...
{% macro default__create_schema(relation) -%}
  ...
  {%- call statement('create_schema') -%}
   CREATE SCHEMA {{ relation }} WITH (location = 's3a://trino-dev-datasets-118330671040/hive')
  {% endcall %}
...
{% macro trino__drop_schema(relation) -%}
  {% do log('trino__drop_schema', info=true) %}
...
{% macro trino__rename_relation(from_relation, to_relation) -%}
  {% do log('trino__rename_relation', info=true) %}
...
{% macro trino__alter_relation_comment(relation, relation_comment) -%}
  {% do log('trino__alter_relation_comment', info=true) %}
...
{% macro trino__list_schemas(database) -%}
  {% do log('trino__list_schemas', info=true) %}
  {% do log(information_schema_name(database), info=true) %}
...
{% macro trino__check_schema_exists(information_schema, schema) -%}
  {% do log('trino__check_schema_exists', info=true) %}
