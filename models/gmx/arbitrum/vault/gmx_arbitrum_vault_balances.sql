...
WITH minute AS  -- This CTE generates a series of minute values
    ...
    FROM
        ...
        SELECT explode(sequence(TIMESTAMP CAST('{{project_start_date}}' AS TIMESTAMP(3)), CURRENT_TIMESTAMP, INTERVAL 1 minute)) AS minute -- 2021-08-31 08:13 is the timestamp of the first vault transaction
        {% endif %}
        {% if is_incremental() %}
    ...
    FROM
        ...
        FROM
            ...
            WHERE `to` = 0x489ee077994b6658eafa855c308275ead8097c4a -- GMX Arbitrum Vault Address
                ...
                {% if not is_incremental() %}
                AND evt_block_time >= CAST('{{project_start_date}}' AS TIMESTAMP(3))
                {% endif %}
                {% if is_incremental() %}
