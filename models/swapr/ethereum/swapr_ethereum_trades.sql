...
(
    ...
    {% if is_incremental() %}
    WHERE
    t.evt_block_time >= date_trunc('day', now() - interval '7' day)
    {% endif %}
)
...
LEFT JOIN {{ source('prices', 'usd') }} pa 
    ...
    {% if not is_incremental() %}
    AND pa.minute >= CAST('{{project_start_date}}' AS TIMESTAMP(3))
    {% endif %}
    {% if is_incremental() %}
...
LEFT JOIN {{ source('prices', 'usd') }} pb 
    ...
    {% if not is_incremental() %}
    AND pb.minute >= CAST('{{project_start_date}}' AS TIMESTAMP(3))
    {% endif %}
    {% if is_incremental() %}
