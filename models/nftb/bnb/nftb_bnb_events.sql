...
(SELECT tr.contract_address as project_contract_address
    ...
    --   ,to 
       ...
       ,_recipient as royalty_fee_receive_address
        ,CAST(0 as DOUBLE) AS amount_raw
...
WHERE tr.evt_block_time >= CAST('{{project_start_date}}' AS TIMESTAMP(3))
{% if is_incremental() %}
...
AND  tr.evt_block_time >= CAST('{{project_start_date}}' AS TIMESTAMP(3))
{% if is_incremental() %}
AND tr.evt_block_time >= date_trunc('day', now() - interval '7' day)
{% endif %}
...
AND tr1.block_time >= CAST('{{project_start_date}}' AS TIMESTAMP(3))
{% endif %}
{% if is_incremental() %}
...
LEFT JOIN (SELECT 
            ...
            {% if not is_incremental() %}
            AND bt.block_time >= CAST('{{project_start_date}}' AS TIMESTAMP(3))
            {% endif %}
            {% if is_incremental() %}
...
{% if not is_incremental() %}
AND l1.block_time >= CAST('{{project_start_date}}' AS TIMESTAMP(3))
{% endif %}
{% if is_incremental() %}
...
{% if not is_incremental() %}
AND l2.block_time >= CAST('{{project_start_date}}' AS TIMESTAMP(3))
{% endif %}
{% if is_incremental() %}
...
{% if not is_incremental() %}
AND l3.block_time >= CAST('{{project_start_date}}' AS TIMESTAMP(3))
{% endif %}
{% if is_incremental() %}
...
{% if not is_incremental() %}
AND btx.block_time >= CAST('{{project_start_date}}' AS TIMESTAMP(3))
{% endif %}
{% if is_incremental() %}
...
{% if not is_incremental() %}
AND p.minute >= CAST('{{project_start_date}}' AS TIMESTAMP(3))
{% endif %}
{% if is_incremental() %}
...
{% if not is_incremental() %}
AND erc721.evt_block_time >= CAST('{{project_start_date}}' AS TIMESTAMP(3))
{% endif %}
{% if is_incremental() %}
