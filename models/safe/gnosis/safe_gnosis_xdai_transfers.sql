...
where et.block_time > CAST('{{project_start_date}}' AS TIMESTAMP(3)) -- for initial query optimisation
{% endif %}
{% if is_incremental() %}
...
{% if not is_incremental() %}
where a.evt_block_time > CAST('{{project_start_date}}' AS TIMESTAMP(3)) -- for initial query optimisation
{% endif %}
{% if is_incremental() %}
-- to prevent potential counterfactual safe deployment issues we take a bigger interval
