...
(
    ...
    SELECT
        ...
        CASE WHEN CAST(amount0In AS DOUBLE) = 0 OR amount1Out = 0 THEN amount1In ELSE amount0In END AS token_sold_amount_raw,
        cast(NULL as double)                                                            AS amount_usd,
        CASE WHEN CAST(amount1Out AS DOUBLE) = 0 THEN f.token1 ELSE f.token0 END                      AS token_bought_address,
        CASE WHEN CAST(amount0In AS DOUBLE) = 0 OR amount1Out = 0 THEN f.token1 ELSE f.token0 END   AS token_sold_address,
